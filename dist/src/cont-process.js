...
exports.default = {
    ...
    */
    ...
    closeHost(fileContent, hostName) {
        let targetSnippetReg = this.getRegByHost(hostName), execResult, middleContent;
    ...
    getAllHostName(fileContent) {
        let allHostNameReg = /#=+[\s]?([\d\w_\-\u4E00-\u9FFF]+)/g, matches, allHostNames = [];
    ...
    getActivedHost(fileContent) {
        let execResult, middleContent, eachLineArray, activedHost = [], activeCount = 0, targetSnippetReg = new RegExp('(#=+[\\s]*([\\d\\w\\u4E00-\\u9FFF]+))([^=]+)(#=+)', 'g');
